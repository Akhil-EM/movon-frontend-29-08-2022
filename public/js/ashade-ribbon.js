...
ashade_ribbon = {
	...
	touchSpeed: {
		vertical: document.getElementById("whichpage").value?document.getElementById("whichpage").value:2, // Speed factor for Vertical Ribbon
		...
		ashade_ribbon.$el[0].addEventListener('touchmove', function(e) {
			if (ashade_ribbon.isDown) {
				if (ashade_ribbon.type == 'vertical') {
					...
					{
						document.getElementById("ashade-header").style.top = "0";
                    } else {
                    document.getElementById("ashade-header").style.top = "-130px";
						}
					ashade_ribbon.old_pageX = e.touches[0].clientY;
					if (newTop > ashade_ribbon.maxStep) {
						newTop = ashade_ribbon.maxStep;
